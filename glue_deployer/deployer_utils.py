...
def _build_job_dict(jobfile, s3key, env):
...
def _job_path_from_jobfile(jobfile):
    return JOB_PATH.format(_jobname_from_file(jobfile))


def _upload_jobfile(jobfile, env, s3):
    filelocation = './gluejobs/{}'.format(jobfile)
    data = open(filelocation, 'rb')
    keypath = _job_path_from_jobfile(jobfile)
...
def _create_job_directories(jobfile, env, s3):
...
def _recursive_s3_delete(path_const, jobfile, env, s3_client):
    top_dir = path_const.format(_jobname_from_file(jobfile))
...
def _upload_packages(jobfile, env, s3):
...
def update_or_create_job(jobfile, env):  # pylint:disable=inconsistent-return-statements
    ...
    _upload_packages(jobfile, env, s3_client)
    try:
        if glue_client.get_job(JobName=_jobname_from_file(jobfile)):
            print("Updating Existing Job: {}".format(_jobname_from_file(jobfile)))
            _upload_jobfile(jobfile, env, s3_client)
    except ClientError as e:
        if e.response['Error']['Code'] == 'EntityNotFoundException':
            print("Creating New Job: {}".format(_jobname_from_file(jobfile)))
            s3file = _upload_jobfile(jobfile, env, s3_client)
            _create_job_directories(jobfile, env, s3_client)
            argdict = _build_job_dict(jobfile, _get_s3path(s3file), env)
            return glue_client.create_job(**argdict)


def update_or_create_all_jobs_for_stage(stage, env):
    joblist = _list_jobfiles_for_stage(stage)
    for job in joblist:
        update_or_create_job(job, env)


def destroy_job(jobfile, env):
    session = _init_boto_session()
    glue_client = session.client('glue')
    s3_client = session.client('s3')
    glue_client.delete_job(JobName=_jobname_from_file(jobfile))
    job_key = _job_path_from_jobfile(jobfile)
    job_lib_key = _package_path_from_jobfile(jobfile,
                                             _generate_zip_name_from_jobfile(jobfile))
    for k in [STAGING_PATH, TMP_PATH]:
        _recursive_s3_delete(path_const=k, jobfile=jobfile, env=env, s3_client=s3_client)
...
def destroy_all_jobs_for_stage(stage, env):
    joblist = _list_jobfiles_for_stage(stage)
    for job in joblist:
        destroy_job(job, env)
